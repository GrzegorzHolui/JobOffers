...
//        step 16: user made POST /offers with header “Authorization: Bearer AAAA.BBBB.CCC” and offer and system returned CREATED(201) with saved offer
        ...
        OfferFacadeResultDto resultOfferFacade = objectMapper.readValue
                (mvcResultIfirma.getResponse().getContentAsString(), new TypeReference<>() {
                });

        assertAll(() -> assertThat(mvcResultIfirma.getResponse().getStatus()).isEqualTo(201),
                () -> assertThat(resultOfferFacade.offerFacade().linkToOffer())
                        .isEqualTo("https://kariera.ifirma.pl/junir-java-developer"),
